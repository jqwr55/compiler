...
template<typename T>
T* GetPointer(u32* ptr) {
    auto v = *ptr;
    return v ? (T*)((byte*)ptr + v) : nullptr;
}
...
void pool_free(MemoryPool<slab_size>* pool, void* memory) {
    static_assert(slab_size >= sizeof(void*));
    free_list_free(&pool->head, memory);
}
...
void* pool_multi_allocate(MultiAllocPool<slab_size>* pool, u32 size) {
    ...
    auto it = GetPointer0<PoolBlock>(pool->base, pool->head);
    PoolBlock* prev = (PoolBlock*)&pool->head;
    while(it) {
        if(it->size >= size) {
...
void pool_multi_free(MultiAllocPool<slab_size>* pool, void* memory, u32 size) {
    ...
    block->size = size;
    pool->head = (byte*)memory - pool->base;
}
